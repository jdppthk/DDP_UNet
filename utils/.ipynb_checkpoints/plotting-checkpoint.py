...
def plot_gens_tars(gens, tars):
  
  figslist = []
  
  for fig_iter in range(5):
    
    img = fig_iter*5
  
    fig = plt.figure()

    for ch_iter in range(5):

      plt.subplot(5,2,2*ch_iter+1)
      plt.imshow(gens[img][ch_iter,:,:], vmin = -1, vmax = 1)
      plt.colorbar()
      plt.subplot(5,2,2*ch_iter+2)
      plt.imshow(tars[img][ch_iter,:,:], vmin = -1, vmax = 1)
      plt.colorbar()
